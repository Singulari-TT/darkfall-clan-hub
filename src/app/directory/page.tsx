...
import { ArrowLeft, User, Shield, Sword, Activity } from "lucide-react";
    ...
    return (
        <div className="min-h-screen bg-transparent text-gray-300 p-4 sm:p-8 font-sans selection:bg-social-cobalt-dim">
            <div className="max-w-7xl mx-auto space-y-8 relative z-10">
                <header className="border-b border-surface-border pb-6 flex items-center gap-6">
                    <Link
                        href="/"
                    ...
                    <div className="flex-1">
                        <h1 className="text-4xl font-heading font-black text-transparent bg-clip-text bg-gradient-to-r from-red-600 to-[#c5a059] tracking-wider drop-shadow-sm uppercase leading-tight">
                            Clan <span className="text-social-cobalt">Roster</span>
                        </h1>
                        <p className="text-gray-500 font-mono text-[10px] uppercase tracking-widest">Active database identities of the Dreadkrew. Verified credentials only.</p>
                    </div>
                    {session?.user && (
                        <Link href="/profile" className="hidden sm:inline-block bg-social-cobalt/90 hover:bg-social-cobalt text-white font-bold tracking-wider py-3 px-8 rounded-card shadow-[0_0_20px_rgba(88,101,242,0.3)] transition-all">
                            Update Dossier
                        </Link>
                    )}
                </header>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    {sortedUsers.map((user: any) => {
                        ...
                        return (
                            <div key={user.id} className={`bg-surface border ${isCurrentUser ? 'border-social-cobalt shadow-[0_0_20px_rgba(88,101,242,0.2)]' : 'border-surface-border'} rounded-card overflow-hidden hover:border-social-cobalt-border hover:shadow-[0_8px_40px_rgba(0,0,0,0.6)] transition-all duration-500 group backdrop-blur-[--blur-glass] relative flex flex-col min-h-[420px]`}>
                                {/* Header banner with dynamic color */}
                                <div className={`h-[60px] w-full relative overflow-hidden ${isAdminRole ? 'bg-gradient-to-r from-social-cobalt-dim to-black/80' : 'bg-gradient-to-r from-stone-900 to-black'}`}>
                                    <div className="absolute inset-0 opacity-20 bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')]"></div>
                                    {isAdminRole && <div className="absolute top-0 right-0 w-32 h-32 bg-social-cobalt-dim/50 rounded-full blur-3xl pointer-events-none -mt-10 -mr-10"></div>}
                                ...
                                <div className="pt-12 px-6 pb-6 flex-1 flex flex-col relative z-20">
                                    <div className="flex justify-between items-start mb-4">
                                        <div className="flex flex-col">
                                            <div className="flex items-center gap-2">
                                                ...
                                                {isVerified && <span className="text-social-cobalt" title="Identity Verified">
                                            ...
                                            <div className="flex gap-2 items-center flex-wrap">
                                                <span className={`text-[9px] uppercase tracking-widest font-black px-2 py-0.5 rounded border ${isAdminRole ? 'bg-social-cobalt-dim text-social-cobalt border-social-cobalt-border' : 'bg-white/5 text-gray-400 border-surface-border'}`}>
                                                    {user.role}
                                                </span>
                                                {isVerified ? (
                                    ...
                                    <div className="mt-4 pt-4 border-t border-white/5">
                                        <a
                                            ...
                                            className="block w-full text-center py-2 rounded-xl bg-social-cobalt/10 hover:bg-social-cobalt/20 text-social-cobalt border border-social-cobalt/30 text-[10px] font-black uppercase tracking-widest transition-all"
                                        >
                                            Hail Member
                                        </a>
                                    </div>
